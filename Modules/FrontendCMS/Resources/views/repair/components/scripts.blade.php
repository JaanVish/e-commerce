    ...
    (function($){
        ...
        $(document).ready(function() {
            ...
            $(document).on('submit', '#item_create_form', function(event){
                ...
                $.ajax({
                    url: "{{ route('frontendcms.repairs.store')}}",
                    type:"POST",
                    cache: false,
                    contentType: false,
                    processData: false,
            ...
            $(document).on('submit', '#item_edit_form', function(event){
                ...
                formData.append('_token',"{{ csrf_token() }}");
                $.ajax({
                    url: "{{ route('frontendcms.repairs.update')}}",
                    type:"POST",
                    cache: false,
                    contentType: false,
                    processData: false,
            ...
            $(document).on('submit', '#deleteItemModal',function(event){
                ...
                $.ajax({
                    url: "{{ route('frontendcms.repairs.delete')}}",
                    type:"POST",
                    cache: false,
                    contentType: false,
                    processData: false,
            ...
            $(document).on('click', '.edit_repair', function(event){
                event.preventDefault();
                let id = $(this).data('id');
                if(id){
                    $('#pre-loader').removeClass('d-none');
                    ...
                    let url = base_url + '/frontendcms/repairs/edit/' +id;
            ...
            $(document).on('click', '.delete_repair', function(event){
                event.preventDefault();
                let id = $(this).data('id');
                if(id){
